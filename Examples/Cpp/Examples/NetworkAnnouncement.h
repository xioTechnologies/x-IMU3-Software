...
private:
    ...
    {
        std::cout << message.device_name << ", " << message.serial_number << ", " << message.ip_address << ", " << message.tcp_port << ", " << message.udp_send << ", " << message.udp_receive << ", " << message.rssi << "%, " << message.battery << "%, " << XIMU3_charging_status_to_string(message.charging_status) << std::endl;
