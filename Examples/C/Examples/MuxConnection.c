...
void MuxConnection()
{
    ...
    XIMU3_Connection* const connectionInfo = XIMU3_connection_new_mux(mux_connection_info);

    XIMU3_mux_connection_info_free(mux_connection_info);

    Run(connectionInfo, XIMU3_mux_connection_info_to_string(mux_connection_info));

    XIMU3_connection_close(connectionInfo);
    XIMU3_connection_free(connectionInfo);
