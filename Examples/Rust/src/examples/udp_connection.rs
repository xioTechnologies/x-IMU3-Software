...
pub fn run() {
    if helpers::yes_or_no("Connect automatically?") {
        ...
        let Some(config) = Option::<UdpConnectionConfig>::from(message) else {
        ...
        let config = &ConnectionConfig::UdpConnectionConfig(config);

        connection::run(config);
    } else {
        let config = &ConnectionConfig::UdpConnectionConfig(UdpConnectionConfig {
