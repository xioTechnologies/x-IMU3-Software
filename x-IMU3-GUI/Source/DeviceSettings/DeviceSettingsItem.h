...
{
public:
    DeviceSettingsItem(const juce::ValueTree settingsTree_, const std::vector<juce::ValueTree>& settingsFlattened, const juce::ValueTree enumsTree_) : settingsTree(settingsTree_), enumsTree(enumsTree_)
    {
        setLinesDrawnForSubItems(false);

        for (auto child : settingsTree)
        {
            addSubItem(new DeviceSettingsItem(child, settingsFlattened, enumsTree));
        }
    ...
    int getItemHeight() const override
    {
        ...
        if (ApplicationSettings::getSingleton().deviceSettings.hideUnusedSettings && (isStatusSet(settingsTree) == false))
        {
