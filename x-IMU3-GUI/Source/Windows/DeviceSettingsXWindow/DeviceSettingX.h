...
#include <juce_core/juce_core.h> // JSON::parse
#include "Ximu3.hpp"
#include <optional>
#include <string>
#include <utility>
...
struct DeviceSettingX {
    ...
    std::string writeCommand(const std::string &newValue) const {
        if (readOnly) {
            return readCommand();
        }

        const std::string command = "{\"" + key + "\":" + newValue + "}";

        if (juce::JSON::parse(command).isVoid()) {
            // TODO: Use XIMU3_command_message_parse
            return readCommand();
        }
