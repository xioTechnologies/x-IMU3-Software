...
juce::PopupMenu MenuStrip::getCustomCommandMenu()
{
    ...
    addDevices(menu, [&](auto& connectionPanel)
    {
        DialogQueue::getSingleton().pushFront(std::make_unique<SendCommandDialog>("Send Custom Command to " + connectionPanel.getTitle(), connectionPanel.getTag()), [&, connectionPanel_ = &connectionPanel]
        {
            if (auto* dialog = dynamic_cast<SendCommandDialog*>(DialogQueue::getSingleton().getActive()))
            {
