...
#include "Windows/DeviceSettingsXWindow/DeviceSettingsXWindow.h"
...
#include "Windows/Graphs/SyncGraphWindow.h"
...
#include "Windows/Graphs/ButtonGraphWindow.h"
...
std::shared_ptr<Window> ConnectionPanel::getOrCreateWindow(const juce::ValueTree &windowTree) {
    ...
    if (type == WindowIds::DeviceSettingsX) {
        return window = std::make_shared<DeviceSettingsXWindow>(windowLayout, type, *this);
    }
    ...
    if (type == WindowIds::Ltc) {
        return window = std::make_shared<LtcWindow>(windowLayout, type, *this);
    }
    ...
    if (type == WindowIds::Sync) {
        return window = std::make_shared<SyncGraphWindow>(windowLayout, type, *this, openGLRenderer);
    }
    ...
    if (type == WindowIds::Button) {
        return window = std::make_shared<ButtonGraphWindow>(windowLayout, type, *this, openGLRenderer);
    }
