...
void SerialAccessoryTerminal::addRX(const uint64_t timestamp, const juce::String& text)
{
    ...
    for (const auto& string : splitUnquotedJsonString(text))
    {
...
std::vector<juce::String> SerialAccessoryTerminal::splitUnquotedJsonString(const juce::String& input)
{
    std::vector<juce::String> output(1);

    for (int index = 0; index < (int) input.getNumBytesAsUTF8();)
    {
        if (input[index] != '\\')
        {
            output.back() += input[index++];
            continue;
        }
