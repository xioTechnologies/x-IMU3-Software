...
void AboutDialog::resized()
{
    ...
    const auto labelWidth = std::reduce(rows.begin(), rows.end(), 0, [&] (auto width, const auto& pair) { return std::max(width, juce::GlyphArrangement::getStringWidthInt(UIFonts::getDefaultFont(), pair.first->getText())); });
    const auto valueWidth = std::reduce(rows.begin(), rows.end(), 0, [&] (auto width, const auto& pair) { return std::max(width, juce::GlyphArrangement::getStringWidthInt(UIFonts::getDefaultFont(), pair.second->getText())); });
