...
static PyObject *connection_get_status(Connection *self, PyObject *args) {
    const XIMU3_ConnectionStatus status = XIMU3_connection_get_status(self->wrapped);
...
static PyObject *connection_get_inertial_message(Connection *self, PyObject *args, PyObject *kwds) {
    int consume = false;
    ...
    const XIMU3_InertialMessage message = XIMU3_connection_get_inertial_message(self->wrapped, (bool) consume);

    if (message.timestamp == 0) {
        Py_RETURN_NONE;
    }

    return inertial_message_from(&message);
}

static PyObject *connection_get_magnetometer_message(Connection *self, PyObject *args, PyObject *kwds) {
    int consume = false;
    ...
    const XIMU3_MagnetometerMessage message = XIMU3_connection_get_magnetometer_message(self->wrapped, (bool) consume);
...
static PyObject *connection_get_quaternion_message(Connection *self, PyObject *args, PyObject *kwds) {
    ...
    const XIMU3_QuaternionMessage message = XIMU3_connection_get_quaternion_message(self->wrapped, (bool) consume);
...
static PyObject *connection_get_rotation_matrix_message(Connection *self, PyObject *args, PyObject *kwds) {
    int consume = false;
    ...
    const XIMU3_RotationMatrixMessage message = XIMU3_connection_get_rotation_matrix_message(self->wrapped, (bool) consume);

    if (message.timestamp == 0) {
        Py_RETURN_NONE;
    }

    return rotation_matrix_message_from(&message);
}

static PyObject *connection_get_euler_angles_message(Connection *self, PyObject *args, PyObject *kwds) {
    int consume = false;
    ...
    const XIMU3_EulerAnglesMessage message = XIMU3_connection_get_euler_angles_message(self->wrapped, (bool) consume);
...
static PyObject *connection_get_linear_acceleration_message(Connection *self, PyObject *args, PyObject *kwds) {
    int consume = false;
    ...
    const XIMU3_LinearAccelerationMessage message = XIMU3_connection_get_linear_acceleration_message(self->wrapped, (bool) consume);

    if (message.timestamp == 0) {
        Py_RETURN_NONE;
    }

    return linear_acceleration_message_from(&message);
}

static PyObject *connection_get_earth_acceleration_message(Connection *self, PyObject *args, PyObject *kwds) {
    int consume = false;
    ...
    const XIMU3_EarthAccelerationMessage message = XIMU3_connection_get_earth_acceleration_message(self->wrapped, (bool) consume);

    if (message.timestamp == 0) {
        Py_RETURN_NONE;
    }

    return earth_acceleration_message_from(&message);
}

static PyObject *connection_get_ahrs_status_message(Connection *self, PyObject *args, PyObject *kwds) {
    int consume = false;
    ...
    const XIMU3_AhrsStatusMessage message = XIMU3_connection_get_ahrs_status_message(self->wrapped, (bool) consume);
...
static PyObject *connection_get_temperature_message(Connection *self, PyObject *args, PyObject *kwds) {
    ...
    const XIMU3_TemperatureMessage message = XIMU3_connection_get_temperature_message(self->wrapped, (bool) consume);

    if (message.timestamp == 0) {
        Py_RETURN_NONE;
    }

    return temperature_message_from(&message);
}

static PyObject *connection_get_battery_message(Connection *self, PyObject *args, PyObject *kwds) {
    int consume = false;
    ...
    const XIMU3_BatteryMessage message = XIMU3_connection_get_battery_message(self->wrapped, (bool) consume);

    if (message.timestamp == 0) {
        Py_RETURN_NONE;
    }

    return battery_message_from(&message);
}

static PyObject *connection_get_rssi_message(Connection *self, PyObject *args, PyObject *kwds) {
    int consume = false;
    ...
    const XIMU3_RssiMessage message = XIMU3_connection_get_rssi_message(self->wrapped, (bool) consume);
...
static PyObject *connection_get_notification_message(Connection *self, PyObject *args, PyObject *kwds) {
    ...
    const XIMU3_NotificationMessage message = XIMU3_connection_get_notification_message(self->wrapped, (bool) consume);

    if (message.timestamp == 0) {
        Py_RETURN_NONE;
    }

    return notification_message_from(&message);
}

static PyObject *connection_get_error_message(Connection *self, PyObject *args, PyObject *kwds) {
    int consume = false;
    ...
    const XIMU3_ErrorMessage message = XIMU3_connection_get_error_message(self->wrapped, (bool) consume);

    if (message.timestamp == 0) {
        Py_RETURN_NONE;
    }

    return error_message_from(&message);
}
...
static PyObject *connection_add_status_callback(Connection *self, PyObject *arg) {
    if (PyCallable_Check(arg) == 0) {
        PyErr_SetString(PyExc_TypeError, "'callback' must be callable");
        return NULL;
    }
    ...
    uint64_t id;
    Py_BEGIN_ALLOW_THREADS // avoid deadlock caused by PyGILState_Ensure in callbacks
        id = XIMU3_connection_add_status_callback(self->wrapped, connection_status_callback, arg);
    Py_END_ALLOW_THREADS
...
static PyMethodDef connection_methods[] = {
    ...
    {"get_status", (PyCFunction) connection_get_status, METH_NOARGS, ""},
    ...
    {"add_status_callback", (PyCFunction) connection_add_status_callback, METH_O, ""},
