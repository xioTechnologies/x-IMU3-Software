...
static PyObject *connection_new(PyTypeObject *subtype, PyObject *args, PyObject *kwds) {
    ...
    PyErr_SetString(PyExc_TypeError, "Expected *ConnectionInfo");
    return NULL;
}
...
static PyObject *connection_send_commands(Connection *self, PyObject *args) {
    ...
    for (uint32_t index = 0; index < length; index++) {
        ...
        if (PyUnicode_Check(command) == 0) {
            PyErr_SetString(PyExc_TypeError, "'commands' items must be strings");
            return NULL;
        }
...
static PyObject *connection_add_receive_error_callback(Connection *self, PyObject *args) {
    ...
    uint64_t id;
    Py_BEGIN_ALLOW_THREADS // avoid deadlock caused by PyGILState_Ensure in callbacks
        id = XIMU3_connection_add_receive_error_callback(self->connection, receive_error_callback, callable);
    Py_END_ALLOW_THREADS
...
static PyObject *connection_remove_callback(Connection *self, PyObject *args) {
    ...
    if (PyArg_ParseTuple(args, "K", &callback_id) == 0) {
        return NULL;
    }
