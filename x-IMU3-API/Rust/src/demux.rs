...
impl DeviceEmulator {
    pub fn new(channel: u8) -> Result<DeviceEmulator, ()> {
        ...
        let udp_receive = 8000 + channel as u16;
        let udp_send = 9000 + channel as u16;

        let udp_socket = UdpSocket::bind(SocketAddr::new("0.0.0.0".parse::<IpAddr>().unwrap(), udp_receive)).map_err(|_| ())?;
    ...
    pub fn udp_receive(&self, buffer: &mut [u8]) -> std::io::Result<(usize, SocketAddr)> {
        self.udp_socket.recv_from(buffer)
    }
...
impl Demux<'_> {
    pub fn new(connection: &Connection, channels: Vec<u8>) -> Result<Demux, ()> {
        let devices: HashMap<u8, DeviceEmulator> = channels.into_iter().map(|channel| Ok((channel, DeviceEmulator::new(channel)?))).collect::<Result<_, _>>()?;

        let network_announcement_socket = UdpSocket::bind("0.0.0.0:0").map_err(|_| ())?;
