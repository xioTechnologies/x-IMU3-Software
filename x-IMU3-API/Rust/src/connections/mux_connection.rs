...
impl GenericConnection for MuxConnection {
    fn open(&mut self) -> std::io::Result<()> {
        ...
        let connection_decoder = self.connection_info.connection.lock().unwrap().get_decoder();
        let connection_write_sender = self.connection_info.connection.lock().unwrap().get_write_sender();
        ...
        std::thread::spawn(move || {
            let closure_id = connection_decoder.lock().unwrap().dispatcher.add_mux_closure(Box::new(move |message| {
            ...
            while close_receiver.try_recv().is_err() {
                while let Ok(data) = write_receiver.try_recv() {
                    ...
                    if let Some(connection_write_sender) = &connection_write_sender {
                        connection_write_sender.send(message.into_bytes()).ok();
                    }
                }
            }

            connection_decoder.lock().unwrap().dispatcher.remove_closure(closure_id);
        });
