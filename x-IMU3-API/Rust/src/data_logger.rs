...
impl<'a> DataLogger<'a> {
    pub fn new(destination: &str, name: &str, connections: Vec<&'a Connection>) -> std::io::Result<Self> {
        ...
        for (index, connection) in data_logger.connections.iter().enumerate() {
            ...
            data_logger.closure_ids[index].push(connection.add_receive_error_closure(Box::new(move |decode_error| {
                sender_clone.send((path_clone.join("ReceiveError.txt"), "".to_owned(), decode_error.to_string() + "\n")).ok();
            })));
