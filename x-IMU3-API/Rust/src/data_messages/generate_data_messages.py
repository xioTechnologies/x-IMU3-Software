...
for message in messages:
    if message["argumentNames"] != []:
        ...
        argument_names = message["argumentNames"]
        argument_units = message["argumentUnits"]
    ...
    template = template.replace("$NamePascalCase$", helpers.pascal_case(message["name"]))
    template = template.replace("$NameSnakeCase$", helpers.snake_case(message["name"]))
    template = template.replace("$AsciiID$", message["asciiID"])
...
def insert(file_path, template, id):
    ...
    for message in messages:
        ...
        modifed = modifed.replace("$NameCamelCase$", helpers.camel_case(message["name"]))
        modifed = modifed.replace("$NamePascalCase$", helpers.pascal_case(message["name"]))
        modifed = modifed.replace("$NameSnakeCase$", helpers.snake_case(message["name"]))
...
template = "pub use self::$NameSnakeCase$_message::*;\n"

insert(file_path, template, 0)

template = "pub mod $NameSnakeCase$_message;\n"
...
for message in messages:
    ...
    for name in message["argumentNames"]:
