...
#[no_mangle]
pub extern "C" fn XIMU3_connection_get_ping_response(connection: *mut Connection) -> PingResponseC {
    let connection = unsafe { &*connection };

    connection.get_ping_response().into()
}
...
#[no_mangle]
pub extern "C" fn XIMU3_connection_add_ping_callback(connection: *mut Connection, callback: Callback<PingResponseC>, context: *mut c_void) -> u64 {
    let connection = unsafe { &*connection };
    let void_ptr = VoidPtr(context);

    connection.add_ping_closure(Box::new(move |response| callback(Some(response).into(), void_ptr.0)))
}
