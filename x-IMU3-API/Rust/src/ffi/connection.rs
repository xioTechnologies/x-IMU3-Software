...
#[no_mangle]
pub extern "C" fn XIMU3_connection_add_close_callback(connection: *mut Connection, callback: extern "C" fn(context: *mut c_void), context: *mut c_void) -> u64 {
    let connection = unsafe { &*connection };
    let void_ptr = VoidPtr(context);
    connection.add_close_closure(Box::new(move || callback(void_ptr.0)))
}
