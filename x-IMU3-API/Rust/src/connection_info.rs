...
impl fmt::Display for ConnectionInfo {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ConnectionInfo::MuxConnectionInfo(connection_info) => connection_info.fmt(formatter),
        }
    }
}
...
impl fmt::Display for MuxConnectionInfo {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(formatter, "Mux {}, {}", self.channel, self.connection.lock().unwrap().get_info().to_string())
    }
}
