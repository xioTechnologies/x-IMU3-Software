...
impl Connection {
    ...
    pub(crate) fn ping_internal(internal: &InternalConnection) -> Option<PingResponse> {
        Self::send_commands_internal(internal, vec!["{\"ping\":null}".into()], 4, 200).first()?.as_ref()?;

        internal.lock().unwrap().get_receiver().lock().unwrap().dispatcher.ping_response.lock().unwrap().clone()
    }
    ...
    pub fn get_ping_response(&self) -> Option<PingResponse> {
        self.internal.lock().unwrap().get_receiver().lock().unwrap().dispatcher.ping_response.lock().unwrap().clone()
    }
    ...
    pub fn add_ping_closure(&self, closure: Box<dyn Fn(PingResponse) + Send>) -> u64 {
        self.internal.lock().unwrap().get_receiver().lock().unwrap().dispatcher.add_ping_closure(closure)
    }
