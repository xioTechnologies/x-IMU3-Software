...
impl Dispatcher {
    ...
    pub fn add_decode_error_closure(&self, closure: Box<dyn Fn(DecodeError) + Send>) -> u64 {        println!("add_decode_error_closure");
        let id = self.get_closure_id();
        self.decode_error_closures.lock().unwrap().push((closure, id));
        id
    }

    pub fn add_statistics_closure(&self, closure: Box<dyn Fn(Statistics) + Send>) -> u64 {         println!("add_statistics_closure");
        let id = self.get_closure_id();
        self.statistics_closures.lock().unwrap().push((closure, id));
        id
    }

    pub fn add_command_closure(&self, closure: Box<dyn Fn(CommandMessage) + Send>) -> u64 {         println!("add_command_closure");
        let id = self.get_closure_id();
        self.command_closures.lock().unwrap().push((closure, id));
        id
    }

    pub fn add_data_closure(&self, closure: Box<dyn Fn(Box<dyn DataMessage>) + Send>) -> u64 {         println!("add_data_closure");
        let id = self.get_closure_id();
    ...
    pub fn add_inertial_closure(&self, closure: Box<dyn Fn(InertialMessage) + Send>) -> u64 {         println!("add_inertial_closure");
        let id = self.get_closure_id();
        self.inertial_closures.lock().unwrap().push((closure, id));
        id
    }

    pub fn add_magnetometer_closure(&self, closure: Box<dyn Fn(MagnetometerMessage) + Send>) -> u64 {         println!("add_magnetometer_closure");
        let id = self.get_closure_id();
        self.magnetometer_closures.lock().unwrap().push((closure, id));
        id
    }

    pub fn add_quaternion_closure(&self, closure: Box<dyn Fn(QuaternionMessage) + Send>) -> u64 {         println!("add_quaternion_closure");
        let id = self.get_closure_id();
        self.quaternion_closures.lock().unwrap().push((closure, id));
        id
    }

    pub fn add_rotation_matrix_closure(&self, closure: Box<dyn Fn(RotationMatrixMessage) + Send>) -> u64 {         println!("add_rotation_matrix_closure");
        let id = self.get_closure_id();
        self.rotation_matrix_closures.lock().unwrap().push((closure, id));
        id
    }

    pub fn add_euler_angles_closure(&self, closure: Box<dyn Fn(EulerAnglesMessage) + Send>) -> u64 {         println!("add_euler_angles_closure");
        let id = self.get_closure_id();
        self.euler_angles_closures.lock().unwrap().push((closure, id));
        id
    }

    pub fn add_linear_acceleration_closure(&self, closure: Box<dyn Fn(LinearAccelerationMessage) + Send>) -> u64 {         println!("add_linear_acceleration_closure");
        let id = self.get_closure_id();
        self.linear_acceleration_closures.lock().unwrap().push((closure, id));
        id
    }

    pub fn add_earth_acceleration_closure(&self, closure: Box<dyn Fn(EarthAccelerationMessage) + Send>) -> u64 {         println!("add_earth_acceleration_closure");
        let id = self.get_closure_id();
        self.earth_acceleration_closures.lock().unwrap().push((closure, id));
        id
    }

    pub fn add_ahrs_status_closure(&self, closure: Box<dyn Fn(AhrsStatusMessage) + Send>) -> u64 {         println!("add_ahrs_status_closure");
        let id = self.get_closure_id();
        self.ahrs_status_closures.lock().unwrap().push((closure, id));
        id
    }

    pub fn add_high_g_accelerometer_closure(&self, closure: Box<dyn Fn(HighGAccelerometerMessage) + Send>) -> u64 {         println!("add_high_g_accelerometer_closure");
        let id = self.get_closure_id();
        self.high_g_accelerometer_closures.lock().unwrap().push((closure, id));
        id
    }

    pub fn add_temperature_closure(&self, closure: Box<dyn Fn(TemperatureMessage) + Send>) -> u64 {         println!("add_temperature_closure");
        let id = self.get_closure_id();
        self.temperature_closures.lock().unwrap().push((closure, id));
        id
    }

    pub fn add_battery_closure(&self, closure: Box<dyn Fn(BatteryMessage) + Send>) -> u64 {         println!("add_battery_closure");
        let id = self.get_closure_id();
        self.battery_closures.lock().unwrap().push((closure, id));
        id
    }

    pub fn add_rssi_closure(&self, closure: Box<dyn Fn(RssiMessage) + Send>) -> u64 {         println!("add_rssi_closure");
        let id = self.get_closure_id();
        self.rssi_closures.lock().unwrap().push((closure, id));
        id
    }

    pub fn add_serial_accessory_closure(&self, closure: Box<dyn Fn(SerialAccessoryMessage) + Send>) -> u64 {         println!("add_serial_accessory_closure");
        let id = self.get_closure_id();
        self.serial_accessory_closures.lock().unwrap().push((closure, id));
        id
    }

    pub fn add_notification_closure(&self, closure: Box<dyn Fn(NotificationMessage) + Send>) -> u64 {         println!("add_notification_closure");
        let id = self.get_closure_id();
        self.notification_closures.lock().unwrap().push((closure, id));
        id
    }

    pub fn add_error_closure(&self, closure: Box<dyn Fn(ErrorMessage) + Send>) -> u64 {         println!("add_error_closure");
        let id = self.get_closure_id();
    ...
    pub fn add_end_of_file_closure(&self, closure: Box<dyn Fn() + Send>) -> u64 {         println!("add_end_of_file_closure");
        let id = self.get_closure_id();
