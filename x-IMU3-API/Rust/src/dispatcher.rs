...
pub struct Dispatcher {
    ...
    connection_status_closures: Arc<Mutex<Vec<(Box<dyn Fn(ConnectionStatus) + Send>, u64)>>>,
...
impl Dispatcher {
    pub fn new() -> Dispatcher {
        ...
        let dispatcher = Dispatcher {
            ...
            connection_status_closures: Arc::new(Mutex::new(Vec::new())),
        ...
        let connection_status_closures = dispatcher.connection_status_closures.clone();
        ...
        std::thread::spawn(move || loop {
            match receiver.recv() {
                Ok(data) => {
                    match data {
                        DispatcherData::ConnectionStatus(connection_status) => connection_status_closures.lock().unwrap().iter().for_each(|(closure, _)| closure(connection_status)),
    ...
    pub fn add_connection_status_closure(&self, closure: Box<dyn Fn(ConnectionStatus) + Send>) -> u64 {
        ...
        self.connection_status_closures.lock().unwrap().push((closure, id));
        id
    }
    ...
    pub fn remove_closure(&self, closure_id: u64) {
        self.connection_status_closures.lock().unwrap().retain(|(_, id)| id != &closure_id);
