...
pub struct Dispatcher {
    pub incoming_sender: Sender<DispatcherData>,
    closure_counter: u64,
    ...
    high_g_closures: Arc<Mutex<Vec<(Box<dyn Fn(HighGMessage) + Send>, u64)>>>,
    ...
    serial_closures: Arc<Mutex<Vec<(Box<dyn Fn(SerialMessage) + Send>, u64)>>>,
...
impl Dispatcher {
    pub fn new() -> Dispatcher {
        let (incoming_sender, incoming_receiver) = crossbeam::channel::unbounded();
        ...
        let dispatcher = Dispatcher {
            ...
            high_g_closures: Arc::new(Mutex::new(Vec::new())),
            ...
            serial_closures: Arc::new(Mutex::new(Vec::new())),
        ...
        let high_g_closures = dispatcher.high_g_closures.clone();
        ...
        let serial_closures = dispatcher.serial_closures.clone();
            ...
            match incoming_receiver.recv() {
                Ok(data) => {
                    match data {
                        ...
                        DispatcherData::HighG(message) => {
                            data_closures.lock().unwrap().iter().for_each(|(closure, _)| closure(Box::new(message)));
                            high_g_closures.lock().unwrap().iter().for_each(|(closure, _)| closure(message));
                        }
                        ...
                        DispatcherData::Serial(message) => {
                            data_closures.lock().unwrap().iter().for_each(|(closure, _)| closure(Box::new(message)));
                            serial_closures.lock().unwrap().iter().for_each(|(closure, _)| closure(message));
                        }
    ...
    pub fn add_command_closure(&mut self, closure: Box<dyn Fn(CommandMessage) + Send>) -> u64 {
        let id = self.get_closure_id();
        self.command_closures.lock().unwrap().push((closure, id));
        id
    }

    pub fn add_data_closure(&mut self, closure: Box<dyn Fn(Box<dyn DataMessage>) + Send>) -> u64 {
        let id = self.get_closure_id();
    ...
    pub fn add_high_g_closure(&mut self, closure: Box<dyn Fn(HighGMessage) + Send>) -> u64 {
        let id = self.get_closure_id();
        self.high_g_closures.lock().unwrap().push((closure, id));
        id
    }
    ...
    pub fn add_serial_closure(&mut self, closure: Box<dyn Fn(SerialMessage) + Send>) -> u64 {
        let id = self.get_closure_id();
        self.serial_closures.lock().unwrap().push((closure, id));
        id
    }
    ...
    pub fn remove_closure(&mut self, closure_id: u64) {
        ...
        self.high_g_closures.lock().unwrap().retain(|(_, id)| id != &closure_id);
        ...
        self.serial_closures.lock().unwrap().retain(|(_, id)| id != &closure_id);
