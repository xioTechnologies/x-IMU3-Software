...
pub struct Dispatcher {
    ...
    close_closures: Arc<Mutex<Vec<(Box<dyn Fn() + Send>, u64)>>>,
}
...
impl Dispatcher {
    pub fn new() -> Self {
        ...
        let dispatcher = Self {
            ...
            close_closures: Arc::new(Mutex::new(Vec::new())),
        };
        ...
        let close_closures = dispatcher.close_closures.clone();
        ...
        std::thread::spawn(move || loop {
            ...
            match data {
                ...
                DispatcherData::Close() => close_closures.lock().unwrap().iter().for_each(|(closure, _)| closure()),
            }
        });
    ...
    pub fn remove_closure(&self, closure_id: u64) {
        ...
        self.close_closures.lock().unwrap().retain(|(_, id)| id != &closure_id);
    }

    pub fn add_close_closure(&self, closure: Box<dyn Fn() + Send>) -> u64 {
        let id = self.get_closure_id();
        self.close_closures.lock().unwrap().push((closure, id));
        id
    }
