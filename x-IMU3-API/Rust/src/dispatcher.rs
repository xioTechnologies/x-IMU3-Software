...
pub struct Dispatcher {
    ...
    response_closures: Arc<Mutex<Vec<(Box<dyn Fn(Response) + Send>, u64)>>>,
...
impl Dispatcher {
    pub fn new() -> Self {
        ...
        let dispatcher = Self {
            ...
            response_closures: Arc::new(Mutex::new(Vec::new())),
        ...
        let response_closures = dispatcher.response_closures.clone();
            ...
            match receiver.recv() {
                Ok(data) => {
                    match data {
                        ...
                        DispatcherData::Response(response) => response_closures.lock().unwrap().iter().for_each(|(closure, _)| closure(response.clone())),
    ...
    pub fn add_response_closure(&self, closure: Box<dyn Fn(Response) + Send>) -> u64 {
        let id = self.get_closure_id();
        self.response_closures.lock().unwrap().push((closure, id));
        id
    }
    ...
    pub fn remove_closure(&self, closure_id: u64) {
        ...
        self.response_closures.lock().unwrap().retain(|(_, id)| id != &closure_id);
